...
# ─── Helper: convert absolute path → relative URL path ────────────────────────
def to_relative_path(absolute_path):
    """Return 'temp/filename.jpg' from any absolute path ending in temp/"""
    if absolute_path is None:
        return None
    return f"temp/{Path(absolute_path).name}"
# ──────────────────────────────────────────────────────────────────────────────
...
async def analyze_document(file: UploadFile = File(...)):
    ...
    # Hybrid image score
    image_score = int(0.6 * cnn_image_score + 0.4 * cv_image_score)
    ...
    return FraudResult(
        ...
        decision=decision,
        flaggedPage=flagged_page,
        totalPages=total_pages,
        breakdown=FraudBreakdown(
            image=image_score,
            text=text_score,
            meta=meta_score,
            elaHeatmap=to_relative_path(ela_path),      
            edgeMap=to_relative_path(edge_path),          
            cloneMap=to_relative_path(clone_path),        
            gradcamMap=to_relative_path(gradcam_path)    
        )
    )
...
async def generate_pdf(result: dict):
    # ← FIXED: use absolute TEMP_DIR so report saves correctly on Render
    output_path = str(TEMP_DIR / f"report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf")
